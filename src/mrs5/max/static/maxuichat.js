...
(function(jq) {
    var views = function() {
        ...
        function MaxPredictive(options) {
            ...
            self.maxuichat = options.maxuichat; 
        ...
        MaxPredictive.prototype.render = function(query, request) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (filter.indexOf(prediction.username) === -1 && prediction_matches_query) {
                    var avatar_url = self.maxuichat.settings.avatarURLpattern.format(prediction.username);
                    var params = {
                    ...
                    predictions = predictions + self.maxuichat.templates.predictive.render(params);
                    selected_index = true;
                }
            }
            if (predictions === '') {
                predictions = '<li>' + self.maxuichat.settings.literals.no_match_found + '</li>';
            }
...
(function(jq) {
    var views = function() {
        ...
        function MaxOverlay(maxuichat) {
            var self = this;
            self.maxuichat = maxuichat;
...
(function(jq) {
    var views = function() {
         ...
         *
         */
        function MaxChatInfo(maxuichat) {
            var self = this;
            self.maxuichat = maxuichat;
            self.title = maxuichat.settings.literals.conversations_info_title;
            ...
            self.displayNameSlot = {
                ...
                save: function() {
                    var $displayName = jq(self.getOwnerSelector('> .maxui-displayname'));
                    var $displayNameInput = jq(self.getOwnerSelector('> #maxui-conversation-displayname-edit input.maxui-displayname'));
                    maxuichat.maxClient.modifyConversation(self.data.id, $displayNameInput.val(), function(event) {
                        self.displayNameSlot.hide();
                        $displayName.text(this.displayName);
                        maxuichat.conversations.messagesview.setTitle(this.displayName);
                    });
                }
            };
        }
        ...
        MaxChatInfo.prototype.bind = function(overlay) {
            ...
            overlay.$el().on('maxui-overlay-close', function(event) {
                var still_exists = _.where(self.maxuichat.conversations.listview.conversations, {
                    id: self.maxuichat.conversations.active
                });
                if (!_.isEmpty(still_exists)) {
                    self.maxuichat.conversations.listview.loadConversation(self.maxuichat.conversations.active);
                }
            });
            ...
            overlay.$el().on('click', self.getSelector('.maxui-participant .maxui-conversation-transfer-to .maxui-icon-ok-circled'), function(event) {
                ...
                self.maxuichat.maxClient.transferConversationOwnership(self.data.id, new_owner_username, function(event) {
            ...
            overlay.$el().on('click', self.getSelector('.maxui-participant .maxui-conversation-kick-user .maxui-icon-ok-circled'), function(event) {
                ...
                self.maxuichat.maxClient.kickUserFromConversation(self.data.id, kicked_username, function(event) {
                    $kicked_user.remove();
                });
            });
            ...
            self.predictive = new max.views.MaxPredictive({
                ...
                source: function(event, query, callback) {
                    self.maxuichat.maxClient.getUsersList(query, callback);
                },
                action: function($selected) {
                    ...
                    var params = {
                        ...
                        literals: self.maxuichat.settings.literals,
                        avatarURL: self.maxuichat.settings.avatarURLpattern.format(username)
                    };
                    var newuser = self.maxuichat.templates.participant.render(params);
            ...
            self.newparticipant = new max.views.MaxInput({
                ...
                delegate: overlay.el,
                placeholder: self.maxuichat.settings.literals.conversations_info_add,
                bindings: {
            ...
            overlay.$el().on('click', self.getOwnerSelector('.maxui-participant .maxui-conversation-add-user .maxui-icon-ok-circled'), function(event) {
                ...
                self.maxuichat.maxClient.addUserToConversation(self.data.id, new_username, function(event) {
                    $participant.animate({
                        opacity: 0
                    }, 200, function(event) {
            ...
            overlay.$el().on('click', self.getSelector('#maxui-conversation-leave .maxui-button'), function(event) {
                var leaving_username = self.maxuichat.settings.username;
                self.maxuichat.maxClient.kickUserFromConversation(self.data.id, leaving_username, function(event) {
                    self.maxuichat.conversations.listview.remove(self.data.id);
                    overlay.hide();
                    jq('#maxuichat-widget-container #maxui-back-conversations a').trigger('click');
                });
            });
            ...
            overlay.$el().on('click', self.getSelector('#maxui-conversation-delete .maxui-help .maxui-confirmation-ok'), function(event) {
                self.maxuichat.maxClient.deleteConversation(self.data.id, function(event) {
                    self.maxuichat.conversations.listview.remove(self.data.id);
                    overlay.hide();
                    jq('#maxuichat-widget-container #maxui-back-conversations a').trigger('click');
                });
        ...
        MaxChatInfo.prototype.load = function(configurator) {
            var self = this;
            self.maxuichat.maxClient.getConversation(self.maxuichat.conversations.active, function(data) {
                self.maxuichat.maxClient.getConversationSubscription(self.maxuichat.conversations.active, self.maxuichat.settings.username, function(subscription) {
                    self.data = data;
                    var participants = [];
                    for (var pt = 0; pt < self.data.participants.length; pt++) {
                        var participant = self.data.participants[pt];
                        participant.avatarURL = self.maxuichat.settings.avatarURLpattern.format(participant.username);
                        participant.owner = participant.username === self.data.owner;
                        participants.push(participant);
                    }
                    var avatar_url = self.maxuichat.settings.conversationAvatarURLpattern.format(self.data.id);
                    ...
                    if (self.data.participants.length <= 2) {
                        ...
                        } else if (self.data.participants[0].username === self.maxuichat.settings.username && self.data.participants.length > 1) {
                            partner = self.data.participants[1];
                        }
                        // User the user partner's avatar as conversation avatar
                        avatar_url = self.maxuichat.settings.avatarURLpattern.format(partner.username);
                    }
                    var params = {
                        displayName: displayName,
                        ...
                        published: self.maxuichat.utils.formatDate(self.data.published, self.maxuichat.language),
                        canManage: self.maxuichat.settings.username === self.data.owner,
                        canAdd: _.contains(subscription.permissions, 'invite')
                    };
                    self.content = self.maxuichat.templates.conversationSettings.render(params);
                    configurator(self);
                });
            });
        };
...
(function(jq) {
    var views = function() {
        ...
        MaxScrollbar.prototype.setHeightChat = function(height) {
            var self = this;
            var wrapper_top = jq('#maxui-conversations .maxui-wrapper').offset().top - self.maxuichat.offset().top - 1;
            self.$bar.css({
                'height': height,
                'top': wrapper_top
            });
...
(function(jq) {
    var views = function() {
        ...
        function MaxConversationsList(maxconversations, options) {
            ...
            self.maxuichat = self.mainview.maxuichat; 
        }
        MaxConversationsList.prototype.load = function(conversations) {
            ...
            if (_.isArray(conversations)) {
                ...
                self.maxuichat.maxClient.getConversationsForUser.apply(self.maxuichat.maxClient, [
                    self.maxuichat.settings.username,
                    function(data) {
                        self.maxuichat.logger.info('Loaded {0} conversations from max'.format(self.maxuichat.settings.username), self.mainview.logtag);
        ...
        MaxConversationsList.prototype.loadConversation = function(conversation_hash) {
            ...
            self.maxuichat.maxClient.getConversationSubscription(conversation_hash, self.maxuichat.settings.username, function(data) {
        ...
        MaxConversationsList.prototype.updateLastMessage = function(conversation_id, message) {
            ...
            self.conversations = _.map(self.conversations, function(conversation) {
                if (conversation.id === conversation_id) {
                    ...
                    if (self.maxuichat.settings.username !== message.user) {
        ...
        MaxConversationsList.prototype.toggle = function() {
            ...
            if (!self.mainview.visible()) {
                ...
                self.mainview.scrollbar.setHeightChat(self.mainview.height - 45);
                ...
                self.maxuichat.settings.conversationsSection = 'conversations';
                literal = self.maxuichat.settings.literals.new_conversation_text;
        ...
        MaxConversationsList.prototype.render = function() {
            ...
            var html = '<span id="maxui-info">' + self.maxuichat.settings.literals.no_chats + '<span>';
            // Render the postbox UI if user has permission
            var showCT = self.maxuichat.settings.UISection === 'conversations';
            var toggleCT = self.maxuichat.settings.disableConversations === false && !showCT;
            var params = {
                avatar: self.maxuichat.settings.avatarURLpattern.format(self.maxuichat.settings.username),
                allowPosting: true,
                buttonLiteral: self.maxuichat.settings.literals.new_message_post,
                textLiteral: self.maxuichat.settings.literals.new_conversation_text,
                literals: self.maxuichat.settings.literals,
                showConversationsToggle: toggleCT ? 'display:block;' : 'display:none;',
                showSubscriptionList: false
            };
            var postbox = self.maxuichat.templates.postBox.render(params);
            ...
            for (var i = 0; i < self.conversations.length; i++) {
                ...
                var avatar_url = self.maxuichat.settings.conversationAvatarURLpattern.format(conversation.id);
                var displayName = '';
                if (conversation.participants.length <= 2) {
                    if (conversation.participants.length === 1) {
                        partner = conversation.participants[0];
                        displayName += '[Archive] ';
                    } else if (conversation.participants[0].username === self.maxuichat.settings.username) {
                ...
                var conv_params = {
                    id: conversation.id,
                    displayName: displayName,
                    text: self.maxuichat.utils.formatText(conversation.lastMessage.content),
                    messages: conversation.unread_messages,
                    literals: self.maxuichat.settings.literals,
                    date: self.maxuichat.utils.formatDate(conversation.lastMessage.published, self.maxuichat.language),
                    avatarURL: avatar_url,
                ...
                html += self.maxuichat.templates.conversation.render(conv_params);
            }
        ...
        MaxConversationMessages.prototype.load = function() {
            ...
            self.maxuichat.maxClient.getMessagesForConversation(conversation_id, {
                limit: 10
            }, function(messages) {
                self.maxuichat.logger.info('Loaded conversation {0} messages from max'.format(conversation_id), self.mainview.logtag);
        ...
        MaxConversationMessages.prototype.ack = function(message) {
            var self = this;
            self.maxuichat.logger.info("Acknowledged Message {0} --> {1}".format(message.uuid, message.data.id), self.mainview.logtag);
            var $message = jq('#maxuichat-widget-container #' + message.uuid);
        ...
        MaxConversationMessages.prototype.loadNew = function() {
            ...
            self.maxuichat.maxClient.getMessagesForConversation(self.mainview.active, {
                limit: 10,
        ...
        MaxConversationMessages.prototype.render = function() {
            ...
            if (self.messages[self.mainview.active]) {
                ...
                for (var i = 0; i < self.messages[self.mainview.active].length; i++) {
                    ...
                    var avatar_url = self.maxuichat.settings.avatarURLpattern.format(message.user.username);
                    ...
                    if (message.user.username === self.maxuichat.settings.username) {
                    ...
                    var params = {
                        id: message.uuid,
                        text: self.maxuichat.utils.formatText(message.data.text),
                        date: self.maxuichat.utils.formatDate(message.published, self.maxuichat.language),
                        ...
                        auth: {
                            'token': self.maxuichat.settings.oAuthToken,
                            'username': self.maxuichat.settings.username
                        }
                    };
                    // Render the conversations template and append it at the end of the rendered covnersations
                    messages = messages + self.maxuichat.templates.message.render(params);
                ...
                _.each(images_to_render, function(message, index, list) {
                    self.maxuichat.maxClient.getMessageImage('/messages/{0}/image/thumb'.format(message.uuid), function(encoded_image_data) {
        ...
        MaxConversationMessages.prototype.show = function(conversation_hash) {
            ...
            var $button = jq('#maxuichat-widget-container #maxui-newactivity').find('input.maxui-button');
        ...
        MaxConversationMessages.prototype.toggle = function() {
            ...
            self.mainview.loadWrappers();
            var literal = '';
            if (self.maxuichat.settings.conversationsSection !== 'messages') {
                self.mainview.$addpeople.animate({
                    'height': 0,
                    'padding-top': 0,
                    'padding-bottom': 0
                ...
                self.mainview.$conversations_list.animate({
                    'margin-left': self.maxuichat.settings.sectionsWidth * (-1)
                }, 400);
                self.mainview.$messages.animate({
                    'left': 0,
                    'margin-left': 0
                ...
                self.mainview.$messages.width(self.maxuichat.settings.sectionsWidth);
                self.maxuichat.settings.conversationsSection = 'messages';
                literal = self.maxuichat.settings.literals.new_activity_text;
                self.mainview.$postbox.val(literal).attr('data-literal', literal);
            }
        };
        /** MaxConversations
         ...
         *
         */
        function MaxConversations(maxuichat, options) {
            var self = this;
            ...
            self.conversationSettings = new max.views.MaxChatInfo(self.maxuichat);
            self.active = '';
        }
        ...
        MaxConversations.prototype.loadScrollbar = function() {
            ...
            self.scrollbar = new max.views.MaxScrollbar({
                maxuichat: self.maxuichat,
                width: self.maxuichat.settings.scrollbarWidth,
                handle: {
                    height: 20
                },
        ...
        MaxConversations.prototype.loadWrappers = function() {
            ...
            self.$newmessagebox = jq('#maxuichat-widget-container #maxui-newactivity');
        };
        ...
        MaxConversations.prototype.bindEvents = function() {
            ...
            jq('#maxuichat-widget-container #maxui-conversation-info').click(function(event) {
                event.preventDefault();
                event.stopPropagation();
                self.maxuichat.overlay.show(self.conversationSettings);
            });
        ...
        MaxConversations.prototype.send = function(text) {
            ...
            var sent = self.maxuichat.messaging.send(message, '{0}.messages'.format(self.active));
        ...
        MaxConversations.prototype.create = function(options) {
            var self = this;
            options.participants.push(self.maxuichat.settings.username);
            self.maxuichat.maxClient.addMessageAndConversation(options, function(event) {
                var message = this;
        ...
        MaxConversations.prototype.ReceiveMessage = function(message) {
            ...
            var message_from_another_user = message.user.username !== self.maxuichat.settings.username;
            ...
            if (message_from_another_user || message_not_in_list) {
                if (message_from_another_user) {
                    self.maxuichat.logger.log('New message from user {0} on {1}'.format(message.user.username, message.destination), self.logtag);
                } else {
                    self.maxuichat.logger.log('Updating {1} messages sent from other {0} instances'.format(message.user.username, message.destination), self.logtag);
                }
                self.messagesview.append(message);
                self.updateUnreadConversations();
                if (self.maxuichat.settings.UISection === 'conversations' && self.maxuichat.settings.conversationsSection === 'messages') {
                    self.messagesview.render(false);
                    self.scrollbar.setContentPosition(100);
                } else if (self.maxuichat.settings.UISection === 'conversations' && self.maxuichat.settings.conversationsSection === 'conversations') {
                    self.listview.render(false);
                } else if (self.maxuichat.settings.UISection === 'timeline') {
                    self.listview.render(false);
                }
            } //else {
        ...
        MaxConversations.prototype.ReceiveConversation = function(message) {
            ...
            var message_not_in_list = self.messagesview.exists(message);
            if (message_from_another_user || message_not_in_list) {
                if (self.maxuichat.settings.UISection === 'conversations' && self.maxuichat.settings.conversationsSection === 'conversations') {
                ...
                } else if (self.maxuichat.settings.UISection === 'conversations' && self.maxuichat.settings.conversationsSection === 'messages') {
                    self.active = message.destination;
                    self.listview.loadConversation(message.destination, function(event) {
...
(function(jq) {
     ...
     *
     */
    function MaxMessaging(maxuichat) {
        var self = this;
        ...
        self.debug = self.maxuichat.settings.enableAlerts;
        self.token = self.maxuichat.settings.oAuthToken;
        self.stompServer = self.maxuichat.settings.maxTalkURL;
        // Construct login merging username with domain (if any)
        // if domain explicitly specified, take it, otherwise deduce it from url
        if (maxuichat.settings.domain) {
            self.domain = maxuichat.settings.domain;
        } else {
            self.domain = self.domainFromMaxServer(self.maxuichat.settings.maxServerURL);
        }
        self.login = "";
        if (self.domain) {
            self.login += self.domain + ':';
        }
        self.login += self.maxuichat.settings.username;
        // Start socket
    ...
    MaxMessaging.prototype.start = function() {
        var self = this;
        self.maxuichat.logger.info('Connecting ...', self.logtag);
        self.connect();
        var current_try = 1;
        // Retry connection if initial failed
        var interval = setInterval(function(event) {
            if (!self.active && current_try <= self.max_retries) {
                self.maxuichat.logger.debug('Connection retry #{0}'.format(current_try), self.logtag);
                self.ws.close();
                self.ws = new SockJS(self.maxuichat.settings.maxTalkURL);
                self.connect();
            } else {
                if (!self.active) {
                    self.maxuichat.logger.error('Connection failure after {0} reconnect attempts'.format(self.max_retries), self.logtag);
                }
    ...
    MaxMessaging.prototype.on_message = function(message, routing_key) {
        ...
        if (_.isEmpty(matched_bindings)) {
            self.maxuichat.logger.warning('Ignoring received message\n{0}\n No binding found for this message'.format(message), self.logtag);
        } else {
            _.each(matched_bindings, function(binding, index, list) {
                self.maxuichat.logger.debug('Matched binding "{1}"'.format(message, binding.key), self.logtag);
    ...
    MaxMessaging.prototype.connect = function() {
        ...
        self.stomp.debug = function(message) {
            self.maxuichat.logger.debug(message, self.logtag);
        };
        var product = 'maxui';
        if (self.maxuichat.settings.generator) {
            product += '[{0}]'.format(self.maxuichat.settings.generator);
        }
        var headers = {
            login: self.login,
            passcode: self.token,
            host: self.vhost,
            product: product,
            "product-version": self.maxuichat.version,
        ...
        var connectCallback = function(x) {
            self.stomp.subscribe('/exchange/{0}.subscribe'.format(self.maxuichat.settings.username), function(stomp_message) {
            ...
            self.active = true;
            self.maxuichat.logger.info('Succesfully connected to {0}'.format(self.stompServer), self.logtag);
        };
        var errorCallback = function(error) {
            self.maxuichat.logger.error(error.body);
        };
    ...
    MaxMessaging.prototype.prepare = function(params) {
        ...
        var base = {
            'source': 'widget',
            'version': self.maxuichat.version,
            'user': {
                'username': self.maxuichat.settings.username,
                'displayname': self.maxuichat.settings.displayName
            },
            'published': self.maxuichat.utils.rfc3339(self.maxuichat.utils.now()),
            'uuid': uuid.v1()
        };
        if (self.domain) {
    ...
    MaxMessaging.prototype.send = function(message, routing_key) {
        ...
        self.stomp.send('/exchange/{0}.publish/{1}'.format(self.maxuichat.settings.username, routing_key), {}, JSON.stringify(self.pack(message_unpacked)));
...
(function(jq) {
    ...
    jq.fn.maxUIChat = function(options) {
        ...
        var maxuichat = this;
        maxuichat.version = '4.1.21';
        maxuichat.templates = max.templates();
        maxuichat.utils = max.utils();
        var defaults = {
        ...
        maxuichat.settings = jq.extend(defaults, options);
        maxuichat.logger = new max.MaxLogging(maxuichat);
        maxuichat.logger.setLevel(maxuichat.settings.loglevel);
        // Configure maxui without CORS if CORS not available
        if (!maxuichat.utils.isCORSCapable()) {
            // IF it has been defined an alias, set as max server url
            if (maxuichat.settings.maxServerURLAlias) {
                maxuichat.settings.maxServerURL = maxuichat.settings.maxServerURLAlias;
            }
        }
        ...
        maxuichat.settings.maxTalkURL = maxuichat.utils.normalizeWhiteSpace(maxuichat.settings.maxTalkURL);
        if (_.isUndefined(maxuichat.settings.maxTalkURL) || maxuichat.settings.maxTalkURL === "") {
            maxuichat.settings.maxTalkURL = maxuichat.settings.maxServerURL + '/stomp';
        }
        // Normalize domain if present, to avoid errors with unvalid values and whitespaces
        maxuichat.settings.domain = maxuichat.utils.normalizeWhiteSpace(maxuichat.settings.domain);
        if (maxuichat.settings.domain.toLowerCase() === 'none') {
            maxuichat.settings.domain = "";
        }
        // Check timeline/activities consistency
        if (maxuichat.settings.UISection === 'timeline' && maxuichat.settings.activitySource === 'timeline' && maxuichat.settings.readContext) {
            maxuichat.settings.readContext = undefined;
            maxuichat.settings.writeContexts = [];
        }
        // Check showSubscriptionList consistency
        if (maxuichat.settings.showSubscriptionList && maxuichat.settings.activitySource === 'activities') {
            maxuichat.settings.showSubscriptionList = false;
        }
        // Get language from options or set default.
        // Set literals in the choosen language and extend from user options
        maxuichat.language = options.language || 'en';
        var user_literals = options.literals || {};
        maxuichat.settings.literals = jq.extend(max.literals(maxuichat.language), user_literals);
        if (maxuichat.settings.readContext) {
            // Calculate readContextHash
            maxuichat.settings.readContextHash = maxuichat.utils.sha1(maxuichat.settings.readContext);
            // Add read context to write contexts
            maxuichat.settings.writeContexts.push(maxuichat.settings.readContext);
            // Store the hashes of the write contexts
            maxuichat.settings.writeContextsHashes = [];
            for (var wc = 0; wc < maxuichat.settings.writeContexts.length; wc++) {
                maxuichat.settings.writeContextsHashes.push(maxuichat.utils.sha1(maxuichat.settings.writeContexts[wc]));
            }
        }
        //set default avatar and profile url pattern if user didn't provide it
        if (!maxuichat.settings.avatarURLpattern) {
            maxuichat.settings.avatarURLpattern = maxuichat.settings.maxServerURL + '/people/{0}/avatar';
        }
        if (!maxuichat.settings.contextAvatarURLpattern) {
            maxuichat.settings.contextAvatarURLpattern = maxuichat.settings.maxServerURL + '/contexts/{0}/avatar';
        }
        if (!maxuichat.settings.conversationAvatarURLpattern) {
            maxuichat.settings.conversationAvatarURLpattern = maxuichat.settings.maxServerURL + '/conversations/{0}/avatar';
        }
        // Disable profileURL by now
        // if (!maxuichat.settings.profileURLpattern)
        //        maxuichat.settings['profileURLpattern'] = maxuichat.settings.maxServerURL+'/profiles/{0}'
        // Catch errors triggered by failed max api calls
        if (maxuichat.settings.enableAlerts) {
        ...
        // Init MAX Client
        maxuichat.maxClient = new MaxClient();
        var maxclient_config = {
            server: maxuichat.settings.maxServerURL,
            mode: maxuichat.settings.maxRequestsAPI,
            username: maxuichat.settings.username,
            token: maxuichat.settings.oAuthToken
        };
        maxuichat.maxClient.configure(maxclient_config);
        // Create a instance of a max messaging client
        // This needs to be before MaxConversations instance creation
        maxuichat.messaging = new max.MaxMessaging(maxuichat);
        // View representing the conversations section
        maxuichat.conversations = new max.views.MaxConversations(maxuichat, {});
        // Bind conversation message receiving
        if (!maxuichat.settings.disableConversations) {
            maxuichat.messaging.bind({
                action: 'add',
                object: 'message'
            }, function(message) {
                maxuichat.conversations.ReceiveMessage(message);
            });
            maxuichat.messaging.bind({
                action: 'add',
                object: 'conversation'
            }, function(message) {
                maxuichat.conversations.ReceiveConversation(message);
            });
            maxuichat.messaging.bind({
                action: 'refresh',
                object: 'conversation'
            }, function(message) {
                maxuichat.conversations.messagesview.load(message.destination);
            });
            maxuichat.messaging.bind({
                action: 'ack',
                object: 'message'
            }, function(message) {
                maxuichat.conversations.messagesview.ack(message);
            });
        }
        // Make settings available to utils package
        maxuichat.utils.setSettings(maxuichat.settings);
        /**
         *
         *
         **/
        jq.fn.hidePostbox = function() {
            var maxuichat = this;
            if (maxuichat.settings.activitySource === 'timeline') {
                if (maxuichat.settings.subscriptionsWrite.length > 0) {
                    return maxuichat.settings.hidePostboxOnTimeline;
                }
            }
            for (var i in maxuichat.settings.subscriptionsWrite) {
                var hash = maxuichat.settings.subscriptionsWrite[i].hash;
                if (hash === maxuichat.settings.readContext) {
                    return maxuichat.settings.hidePostboxOnTimeline;
                }
            }
            return true;
        ...
        this.maxClient.getUserData(maxuichat.settings.username, function(data) {
            //Determine if user can write in writeContexts
            maxuichat.settings.displayName = data.displayName || maxuichat.settings.username;
            ...
            maxuichat.settings.subscriptionsWrite = subscriptionsWrite;
            maxuichat.settings.subscriptions = userSubscriptions;
            if (maxuichat.settings.activitySource === 'timeline' && subscriptionsWrite.length > 0) {
                maxuichat.settings.writeContexts.push(subscriptionsWrite[0].hash);
            }
            // Start messaging only if conversations enabled
            if (!maxuichat.settings.disableConversations) {
                maxuichat.messaging.start();
                jq(window).on('beforeunload', function(event) {
                    var x = maxuichat.messaging.disconnect();
                    return x;
                });
            }
            // render main interface
            var showCT = maxuichat.settings.UISection === 'conversations';
            var showTL = maxuichat.settings.UISection === 'timeline';
            var toggleTL = maxuichat.settings.disableTimeline === false && !showTL;
            var toggleCT = maxuichat.settings.disableConversations === false && !showCT;
            var containerWidth = maxuichat.width() - maxuichat.settings.scrollbarWidth;
            var showRecentOrder = maxuichat.settings.activitySortView === 'recent';
            var showLikesOrder = maxuichat.settings.activitySortView === 'likes';
            var showFlaggedOrder = maxuichat.settings.activitySortView === 'flagged';
            var params_chat = {
                username: maxuichat.settings.username,
                literals: maxuichat.settings.literals,
                ...
                hidePostbox: maxuichat.hidePostbox()
            };
            var mainuichat = maxuichat.templates.mainUIChat.render(params_chat);
            maxuichat.html(mainuichat);
            maxuichat.overlay = new max.views.MaxOverlay(maxuichat);
            // Define widths
            // XXX TODO :Read from renderer styles, not hardcoded values
            maxuichat.settings.widgetWidth = maxuichat.width();
            maxuichat.settings.sectionsWidth = maxuichat.settings.widgetWidth - maxuichat.settings.scrollbarWidth - maxuichat.settings.widgetBorder;
            // First-rendering of conversations list, even if it's not displayed on start
            if (!maxuichat.settings.disableConversations) {
                maxuichat.conversations.render();
                maxuichat.conversations.listview.load(data.talkingIn);
            }
            if (maxuichat.settings.UISection === 'conversations') {
                maxuichat.bindEvents();
                maxuichat.toggleSectionChat('conversations');
            } else if (maxuichat.settings.UISection === 'timeline') {
                var sort_orders_by_view = {
                    recent: maxuichat.settings.activitySortOrder,
                    likes: 'likes',
                    flagged: 'flagged'
                };
                maxuichat.printActivities({
                    sortBy: sort_orders_by_view[maxuichat.settings.activitySortView]
                }, function(event) {
                    maxuichat.bindEvents();
                });
            }
    ...
    jq.fn.bindEvents = function() {
        ...
        jq('#maxuichat-widget-container #maxui-more-activities .maxui-button').click(function(event) {
            ...
            if (jq('#maxuichat-widget-container #maxui-search').hasClass('folded')) {
                maxuichat.loadMoreActivities();
            } else {
                var last_result_id = jq('.maxui-activity:last').attr('id');
                maxuichat.reloadFilters(last_result_id);
            }
        });
        ...
        jq('#maxuichat-widget-container #maxui-search').on('click', '#maxui-favorites-filter', function(event) {
            ...
            if (filterFavorites) {
                ...
                valued_literal = maxuichat.settings.literals.valued_favorited_activity;
                recent_literal = maxuichat.settings.literals.recent_favorited_activity;
            } else {
                maxuichat.delFilter({
                    type: 'favorites',
                    value: true
                });
                valued_literal = maxuichat.settings.literals.valued_activity;
                recent_literal = maxuichat.settings.literals.recent_activity;
            }
        ...
        jq('#maxuichat-widget-container #maxui-subscriptions').on('change', function() {
            ...
            if ($urlContext !== 'timeline') {
                ...
                maxuichat.settings.writeContextsHashes = [];
                // Add read context to write contexts
                maxuichat.settings.writeContexts.push($urlContext);
                // Store the hashes of the write contexts
                for (var wc = 0; wc < maxuichat.settings.writeContexts.length; wc++) {
                    maxuichat.settings.writeContextsHashes.push(maxuichat.utils.sha1(maxuichat.settings.writeContexts[wc]));
                }
                ...
                maxuichat.settings.writeContexts = [];
                maxuichat.settings.writeContextsHashes = undefined;
            }
        });
        ...
        jq('#maxuichat-widget-container #maxui-show-timeline').on('click', function(event) {
            event.preventDefault();
            window.status = '';
            maxuichat.printActivities({}, function(event) {
                maxuichat.toggleSectionChat('timeline');
            });
        });
        ...
        jq('#maxuichat-widget-container #maxui-activities').on('click', '.maxui-action.maxui-favorites', function(event) {
            ...
            if (favorited) {
                maxuichat.maxClient.unfavoriteActivity(activityid, function(event) {
                    $favorites.toggleClass('maxui-favorited', false);
                });
            } else {
                maxuichat.maxClient.favoriteActivity(activityid, function(event) {
        ...
        jq('#maxuichat-widget-container #maxui-activities').on('click', '.maxui-action.maxui-likes', function(event) {
            ...
            if (liked) {
                maxuichat.maxClient.unlikeActivity(activityid, function(event) {
                    $likes.toggleClass('maxui-liked', false);
                });
                $likes_count.text(parseInt($likes_count.text(), 10) - 1);
            } else {
                maxuichat.maxClient.likeActivity(activityid, function(event) {
        ...
        jq('#maxuichat-widget-container #maxui-activities').on('click', '.maxui-action.maxui-flag', function(event) {
            ...
            if (flagged) {
                maxuichat.maxClient.unflagActivity(activityid, function(event) {
                    $flag.toggleClass('maxui-flagged', false);
                    $activity.toggleClass('maxui-flagged', false);
                    maxuichat.printActivities({});
                });
            } else {
                maxuichat.maxClient.flagActivity(activityid, function(event) {
        ...
        jq('#maxuichat-widget-container #maxui-activities').on('click', '.maxui-actions .maxui-button-delete', function(event) {
            ...
            var $activity = jq(this).closest('.maxui-activity');
            var activityid = $activity.attr('id');
            maxuichat.maxClient.removeActivity(activityid, function(event) {
        ...
        jq('#maxuichat-widget-container #maxui-activities').on('click', '.maxui-comment .maxui-button-delete', function(event) {
            ...
            maxuichat.maxClient.removeActivityComment(activityid, commentid, function() {
        ...
        jq('#maxuichat-widget-container #maxui-add-people-box').on('focusin', selector, function(event) {
            ...
            var normalized = maxuichat.utils.normalizeWhiteSpace(text, false);
            ...
            if (jq('#maxuichat-widget-container #maxui-conversation-predictive:visible').length > 0 && (event.which === 40 || event.which === 38 || event.which === 13 || event.which === 9)) {
                maxuichat.utils.freezeEvent(event);
            }
            ...
            if (normalized === '') {
                ...
                if (maxuichat.settings.canwrite) {
            ...
            var $area = jq(this);
            var $predictive = jq('#maxuichat-widget-container #maxuichat-widget-container #maxui-conversation-predictive');
            var num_predictions = $predictive.find('.maxui-prediction').length;
            var is_predicting = jq('#maxuichat-widget-container #maxuichat-widget-container #maxui-conversation-predictive:visible').length > 0;
            // Up & down
            ...
            {
                if (maxuichat.settings.conversationsSection === 'conversations') {
                    if (match) {
                        $area.attr('class', 'maxui-text-input');
                        if (matchEOL) {
                            $predictive.show();
                            $predictive.html('<ul></ul>');
                            maxuichat.printPredictions(match[1], '#maxui-conversation-predictive');
                        }
                    } else {
                        $predictive.hide();
                        ...
                        if (!text.match(new RegExp('^\\s*@'))) {
                            ...
                            $area.attr('title', maxuichat.settings.literals.post_permission_not_here);
                        }
                    }
                }
            } //1
        ...
        maxuichat.bindActionBehaviour('#maxui-newactivity', '#maxui-newactivity-box', {}, function(text) {
            if (maxuichat.settings.UISection === 'timeline') {
                maxuichat.sendActivity(text);
                jq('#maxuichat-widget-container #maxuichat-widget-container #maxui-search').toggleClass('folded', true);
            } else if (maxuichat.settings.UISection === 'conversations') {
                if (maxuichat.settings.conversationsSection === 'conversations') {
                    var participants_box = jq('#maxuichat-widget-container #maxuichat-widget-container #maxui-new-participants')[0];
                    var participants = [];
                    ...
                    maxuichat.conversations.create(options);
                } else {
                    maxuichat.conversations.send(text);
                }
            }
            ...
            if (key === 40 && is_predicting && num_predictions > 1) {
                ...
                if (maxuichat.settings.UISection === 'timeline') {
                    jq(button).val(maxuichat.settings.literals.new_activity_post);
                }
            } else //1
            {
                if (maxuichat.settings.UISection === 'timeline') {
                    if (match) {
                        jq(button).val(maxuichat.settings.literals.new_message_post);
                        if (matchEOL) {
                            $predictive.show();
                            $predictive.html('<ul></ul>');
                            maxuichat.printPredictions(match[1], '#maxui-newactivity #maxui-predictive');
                        }
                        ...
                        jq(button).val(maxuichat.settings.literals.new_activity_post);
                        $predictive.hide();
                        if (!text.match(new RegExp('^\\s*@')) && !maxuichat.settings.canwrite) {
                            $area.attr('class', 'maxui-text-input error');
                            $area.attr('title', maxuichat.settings.literals.post_permission_unauthorized);
                        }
                    }
                } else if (maxuichat.settings.UISection === 'conversations') {
                    if (maxuichat.settings.conversationsSection === 'conversations') {
                        maxuichat.reloadPersons();
                    } else if (maxuichat.settings.conversationsSection === 'messages') {
                        $predictive.hide();
                        jq(button).removeAttr('disabled');
                        jq(button).attr('class', 'maxui-button');
                        $area.attr('class', 'maxui-text-input');
        ...
        maxuichat.bindActionBehaviour('#maxui-activities', '.maxui-newcommentbox', {}, function(text) {
            var activityid = jq(this).closest('.maxui-activity').attr('id');
            maxuichat.maxClient.addComment(text, activityid, function() {
                jq('#maxuichat-widget-container #activityContainer textarea').val('');
                var activity_id = this.object.inReplyTo[0].id;
                maxuichat.printCommentsForActivity(activity_id);
        ...
        maxuichat.bindActionBehaviour('#maxui-search', '#maxui-search-box', {}, function(text) {
            maxuichat.textSearch(text);
            jq('#maxuichat-widget-container #maxui-search').toggleClass('folded', false);
        ...
        //           if (e.keyCode === 13) {
        //              maxuichat.textSearch(jq(this).attr('value'))
        //              jq('#maxuichat-widget-container #maxui-search').toggleClass('folded',false)
        //           }
        // });
    };
    ...
    jq.fn.bindActionBehaviour = function(delegate, target, options, clickFunction) {
        ...
        jq(delegate).on('focusin', selector, function(event) {
            ...
            if (normalized === '' && !options.ignore_button) {
                ...
                if (maxuichat.settings.canwrite && !options.ignore_button) {
                    jq(button).removeAttr('disabled');
                    jq(button).attr('class', 'maxui-button');
                    jq(this).attr('class', 'maxui-text-input');
                }
    ...
    jq.fn.textSearch = function(text) {
        var maxuichat = this;
        //Normalize spaces
        var normalized = maxuichat.utils.normalizeWhiteSpace(text);
    ...
    jq.fn.reloadFilters = function() {
        ...
        var activity_items = maxuichat.templates.filters.render(params);
        jq('#maxuichat-widget-container #maxui-search-filters').html(activity_items);
        // Accept a optional parameter indicating search start point
        if (arguments.length > 0) {
            filters = maxuichat.getFilters(arguments[0]);
        } else {
            filters = maxuichat.getFilters();
        }
        maxuichat.printActivities({});
    ...
    jq.fn.delFilter = function(filter) {
        var maxuichat = this;
        var deleted = false;
        for (var i = 0; i < maxuichat.filters.length; i++) {
            if (maxuichat.filters[i].value === filter.value & maxuichat.filters[i].type === filter.type) {
                deleted = true;
                maxuichat.filters.splice(i, 1);
            }
        }
        if (deleted) {
            this.reloadFilters();
    ...
    jq.fn.reloadPersons = function() {
        ...
        message = maxuichat.utils.normalizeWhiteSpace(message);
        ...
        displayName = maxuichat.utils.normalizeWhiteSpace(displayName);
        var params = {
            persons: participants_box.people
        };
        var participants_items = maxuichat.templates.participants.render(params);
        jq('#maxuichat-widget-container #maxui-new-participants').html(participants_items);
        jq('#maxuichat-widget-container #maxui-add-people-box .maxui-label .maxui-count').text('({0}/{1})'.format(participants_box.people.length + 1, maxuichat.settings.maximumConversations));
        if (participants_box.people.length > 0) {
            ...
            if (has_more_than_one_participant && !has_a_displayname) {
                ...
                if (displayName === '') {
                    ...
                    $newmessagebox.find('.maxui-error-box').text(maxuichat.settings.literals.post_permission_missing_displayName);
            ...
            $newmessagebox.find('textarea').attr('class', 'maxui-text-input error');
            $newmessagebox.find('.maxui-error-box').text(maxuichat.settings.literals.post_permission_not_enough_participants);
            $newmessagebox.find('.maxui-error-box').width($newmessagebox.find('textarea').width() - 4);
            $newmessagebox.find('.maxui-error-box').animate({
                'bottom': -25
            }, 200);
    ...
    jq.fn.addPerson = function(person) {
        ...
        if (person.username !== maxuichat.settings.username && participants_box.people.length < (maxuichat.settings.maximumConversations - 1)) {
            for (var i = 0; i < participants_box.people.length; i++) {
                if (participants_box.people[i].username === person.username) {
                    already_filtered = true;
                }
    ...
    jq.fn.toggleSectionChat = function(sectionToEnable) {
        ...
        var widgetWidth = maxuichat.width();
        var sectionPadding = 10;
        var widgetBorder = 1;
        var sectionsWidth = widgetWidth - maxuichat.conversations.scrollbar.width - (sectionPadding * 2) - (widgetBorder * 2);
        var height = 320;
        if (sectionToEnable === 'conversations' && maxuichat.settings.currentConversationSection === 'conversations') {
            ...
            maxuichat.settings.UISection = 'conversations';
            $postbutton.val(maxuichat.settings.literals.new_message_post);
            textarea_literal = maxuichat.settings.literals.new_conversation_text;
            $postbox_text.val(textarea_literal).attr('data-literal', textarea_literal);
            $conversationsbutton.hide();
            if (!maxuichat.settings.disableTimeline) {
                $timelinebutton.show();
            }
            maxuichat.conversations.scrollbar.setHeightChat(height - 45);
            maxuichat.conversations.scrollbar.setTarget('#maxui-conversations #maxui-conversations-list');
            $postbox.show();
        }
        if (sectionToEnable === 'timeline') {
            if (maxuichat.settings.showSubscriptionList === true) {
                $subscriptionsSelect.attr('style', 'display:inline;');
            }
            maxuichat.conversations.listview.toggle();
            $timeline.show();
            ...
            maxuichat.settings.UISection = 'timeline';
            $postbutton.val(maxuichat.settings.literals.new_activity_post);
            textarea_literal = maxuichat.settings.literals.new_activity_text;
            $postbox_text.val(textarea_literal).attr('data-literal', textarea_literal);
            if (!maxuichat.settings.disableConversations) {
                $conversationsbutton.show();
            }
            $timelinebutton.hide();
            if (maxuichat.settings.hidePostboxOnTimeline) {
                $postbox.hide();
            }
            if (maxuichat.hidePostbox()) {
                $postbox.hide();
            }
        }
    };
    ...
    jq.fn.sendActivity = function() {
        ...
        func_params.push(maxuichat.settings.writeContexts);
        func_params.push(function() {
            ...
            if (first.length > 0) {
                ...
                maxuichat.printActivities(filter);
            } else {
                maxuichat.printActivities({});
            }
        });
        //Pass generator to activity post if defined
        if (maxuichat.settings.generatorName) {
            func_params.push(maxuichat.settings.generatorName);
        }
        var activityAdder = maxuichat.maxClient.addActivity;
        activityAdder.apply(maxuichat.maxClient, func_params);
    };
    /**
    ...
    jq.fn.printPredictions = function(query, predictive_selector) {
        ...
        func_params.push(function(items) {
            maxuichat.formatPredictions(items, predictive_selector);
        });
    ...
    jq.fn.formatPredictions = function(items, predictive_selector) {
        ...
        for (var i = 0; i < items.length; i++) {
            var prediction = items[i];
            if (prediction.username !== maxuichat.username) {
                var avatar_url = maxuichat.settings.avatarURLpattern.format(prediction.username);
                var params = {
                    username: prediction.username,
                    displayName: prediction.displayName,
                    avatarURL: avatar_url,
                ...
                // Render the conversations template and append it at the end of the rendered covnersations
                predictions = predictions + maxuichat.templates.predictive.render(params);
            }
        }
        if (predictions === '') {
            predictions = '<li>' + maxuichat.settings.literals.no_match_found + '</li>';
        }
    ...
    jq.fn.formatActivities = function(items, insertAt) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (activity.hasOwnProperty('generator')) {
                if (activity.generator !== maxuichat.settings.generatorName) {
            ...
            if (activity.actor.objectType === 'person') {
                avatar_url = maxuichat.settings.avatarURLpattern.format(activity.actor.username);
            } else if (activity.actor.objectType === 'context') {
                avatar_url = maxuichat.settings.contextAvatarURLpattern.format(activity.actor.hash);
            }
                ...
                if (activity.replies.length > 0) {
                    for (var r = 0; r < activity.replies.length; r++) {
                        ...
                        var reply = {
                            id: comment.id,
                            actor: comment.actor,
                            date: maxuichat.utils.formatDate(comment.published, maxuichat.language),
                            text: maxuichat.utils.formatText(comment.content),
                            avatarURL: maxuichat.settings.avatarURLpattern.format(comment.actor.username),
            ...
            var canCommentActivity = maxuichat.settings.canwrite;
            if (activity.contexts) {
                canCommentActivity = maxuichat.canCommentActivity(activity.contexts[0].url);
            }
            var params = {
                id: activity.id,
                ...
                literals: maxuichat.settings.literals,
                date: maxuichat.utils.formatDate(activity.published, maxuichat.language),
                dateLastComment: lastComment,
                text: maxuichat.utils.formatText(activity.object.content),
                replies: replies,
                favorited: activity.favorited,
                likes: activity.likesCount ? activity.likesCount : 0,
                showLikesCount: maxuichat.currentSortOrder === 'likes',
                liked: activity.liked,
                flagged: activity.flagged,
                avatarURL: avatar_url,
                publishedIn: contexts,
                canDeleteActivity: activity.deletable,
                canFlagActivity: maxuichat.settings.canflag,
                via: generator,
            ...
            var partials = {
                comment: maxuichat.templates.comment
            };
            activities = activities + maxuichat.templates.activity.render(params, partials);
        ...
        _.each(images_to_render, function(activity, index, list) {
            maxuichat.maxClient.getMessageImage('/activities/{0}/image/thumb'.format(activity.id), function(encoded_image_data) {
                var imagetag = '<img class="maxui-embedded" alt="" src="data:image/png;base64,{0}" />'.format(encoded_image_data);
    ...
    jq.fn.formatComment = function(items, activity_id) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            comments = comments + maxuichat.templates.comment.render(params);
        }
    ...
    jq.fn.renderPostbox = function() {
        ...
        var toggleCT = maxuichat.settings.disableConversations === false && !showCT;
        var params = {
            avatar: maxuichat.settings.avatarURLpattern.format(maxuichat.settings.username),
            allowPosting: maxuichat.settings.canwrite,
            buttonLiteral: maxuichat.settings.literals.new_activity_post,
            textLiteral: maxuichat.settings.literals.new_activity_text,
            literals: maxuichat.settings.literals,
            showConversationsToggle: toggleCT ? 'display:block;' : 'display:none;',
            showSubscriptionList: maxuichat.settings.showSubscriptionList && maxuichat.settings.subscriptionsWrite.length > 0,
            subscriptionList: maxuichat.settings.subscriptionsWrite
        };
        var postbox = maxuichat.templates.postBox.render(params);
        var $postbox = jq('#maxuichat-widget-container #maxui-newactivity');
        $postbox.html(postbox);
    };
    /**
    ...
    jq.fn.printActivities = function(ufilters) {
        ...
        var filters = maxuichat.getFilters().filters;
        ...
        if (!filters.sortBy) {
            if (jq('#maxuichat-widget-container #maxui-activity-sort .maxui-sort-action.maxui-most-valued').hasClass('active')) {
                ...
                filters.sortBy = maxuichat.settings.activitySortOrder;
            }
        }
        maxuichat.currentSortOrder = filters.sortBy;
        var activityRetriever = null;
        if (maxuichat.settings.activitySource === 'timeline') {
            activityRetriever = this.maxClient.getUserTimeline;
            func_params.push(maxuichat.settings.username);
        } else if (maxuichat.settings.activitySource === 'activities') {
            activityRetriever = this.maxClient.getActivities;
            var options = {
                context: maxuichat.settings.readContextHash,
                tags: maxuichat.settings.contextTagsFilter
            };
        ...
        if (arguments.length > 1) {
            ...
            func_params.push(function(items) {
                ...
                maxuichat.settings.canwrite = true;
                // If we don't have a context, we're in timeline, so we can write
                if (maxuichat.settings.activitySource === 'activities') {
                    maxuichat.maxClient.getContext(maxuichat.settings.readContextHash, function(context) {
                        // Add read context if user is not subscribed to it
                        var subscriptions = maxuichat.settings.subscriptions;
                        ...
                        // the current user
                        for (var wc = 0; wc < maxuichat.settings.writeContexts.length; wc++) {
                            var write_context = maxuichat.settings.writeContextsHashes[wc];
                            if (subscriptions[write_context].permissions) {
                                if (subscriptions[write_context].permissions.write !== true) {
                                    maxuichat.settings.canwrite = false;
                                }
                                if (subscriptions[write_context].permissions.flag === true) {
                                    maxuichat.settings.canflag = true;
                                } else {
                                    maxuichat.settings.canflag = false;
                                }
                            } else {
                                maxuichat.settings.canwrite = false;
                                maxuichat.settings.canflag = false;
                        ...
                        maxuichat.renderPostbox();
                        // format the result items as activities
                        maxuichat.formatActivities(items, insert_at, callback);
                    });
                } else {
                    maxuichat.renderPostbox(items, insert_at, callback);
                    // format the result items as activities
                    maxuichat.formatActivities(items, insert_at, callback);
                }
            });
            ...
            func_params.push(function(items) {
                maxuichat.formatActivities(items, insert_at);
            });
        }
    ...
    jq.fn.printCommentsForActivity = function(activity_id) {
        ...
        func_params.push(function(data) {
            maxuichat.formatComment(data, activity_id);
        });
        this.maxClient.getCommentsForActivity.apply(this.maxClient, func_params);
    };
    jq.maxuichat = function() {};
    jq.maxuichat.settings = function() {
...
// This snippet assumes that a _maxuichat.onReady function is defined by the api consumer and calls it as a final step

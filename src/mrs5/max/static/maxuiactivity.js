...
(function(jq) {
    var views = function() {
        ...
        function MaxPredictive(options) {
            ...
            self.maxuiactivity = options.maxuiactivity;
        ...
        MaxPredictive.prototype.render = function(query, request) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (filter.indexOf(prediction.username) === -1 && prediction_matches_query) {
                    var avatar_url = self.maxuiactivity.settings.avatarURLpattern.format(prediction.username);
                    var params = {
                    ...
                    predictions = predictions + self.maxuiactivity.templates.predictive.render(params);
                    selected_index = true;
                }
            }
            if (predictions === '') {
                predictions = '<li>' + self.maxuiactivity.settings.literals.no_match_found + '</li>';
            }
...
(function(jq) {
    var views = function() {
        ...
        function MaxOverlay(maxuiactivity) {
            var self = this;
            self.maxuiactivity = maxuiactivity;
            ...
            jq(self.el + ' .maxui-close').click(function(event) {
                event.preventDefault();
                event.stopPropagation();
                self.maxuiactivity.overlay.hide();
            });
        }
...
(function(jq) {
    var views = function() {
        ...
        MaxScrollbar.prototype.setHeightActivity = function(height) {
            var self = this;
            var wrapper_top = jq('#maxui-conversations .maxui-wrapper').offset().top - self.maxuiactivity.offset().top - 1;
            self.$bar.css({
                'height': height,
                'top': wrapper_top
            });
...
(function(jq) {
     ...
     *
     */
    function MaxMessaging(maxuiactivity) {
        var self = this;
        ...
        self.debug = self.maxuiactivity.settings.enableAlerts;
        self.token = self.maxuiactivity.settings.oAuthToken;
        self.stompServer = self.maxuiactivity.settings.maxTalkURL;
        // Construct login merging username with domain (if any)
        // if domain explicitly specified, take it, otherwise deduce it from url
        if (maxuiactivity.settings.domain) {
            self.domain = maxuiactivity.settings.domain;
        } else {
            self.domain = self.domainFromMaxServer(self.maxuiactivity.settings.maxServerURL);
        }
        self.login = "";
        if (self.domain) {
            self.login += self.domain + ':';
        }
        self.login += self.maxuiactivity.settings.username;
        // Start socket
    ...
    MaxMessaging.prototype.start = function() {
        var self = this;
        self.maxuiactivity.logger.info('Connecting ...', self.logtag);
        self.connect();
        var current_try = 1;
        // Retry connection if initial failed
        var interval = setInterval(function(event) {
            if (!self.active && current_try <= self.max_retries) {
                self.maxuiactivity.logger.debug('Connection retry #{0}'.format(current_try), self.logtag);
                self.ws.close();
                self.ws = new SockJS(self.maxuiactivity.settings.maxTalkURL);
                self.connect();
            } else {
                if (!self.active) {
                    self.maxuiactivity.logger.error('Connection failure after {0} reconnect attempts'.format(self.max_retries), self.logtag);
                }
    ...
    MaxMessaging.prototype.on_message = function(message, routing_key) {
        ...
        if (_.isEmpty(matched_bindings)) {
            self.maxuiactivity.logger.warning('Ignoring received message\n{0}\n No binding found for this message'.format(message), self.logtag);
        } else {
            _.each(matched_bindings, function(binding, index, list) {
                self.maxuiactivity.logger.debug('Matched binding "{1}"'.format(message, binding.key), self.logtag);
    ...
    MaxMessaging.prototype.connect = function() {
        ...
        self.stomp.debug = function(message) {
            self.maxuiactivity.logger.debug(message, self.logtag);
        };
        var product = 'maxui';
        if (self.maxuiactivity.settings.generator) {
            product += '[{0}]'.format(self.maxuiactivity.settings.generator);
        }
        var headers = {
            login: self.login,
            passcode: self.token,
            host: self.vhost,
            product: product,
            "product-version": self.maxuiactivity.version,
        ...
        var connectCallback = function(x) {
            self.stomp.subscribe('/exchange/{0}.subscribe'.format(self.maxuiactivity.settings.username), function(stomp_message) {
            ...
            self.active = true;
            self.maxuiactivity.logger.info('Succesfully connected to {0}'.format(self.stompServer), self.logtag);
        };
        var errorCallback = function(error) {
            self.maxuiactivity.logger.error(error.body);
        };
    ...
    MaxMessaging.prototype.prepare = function(params) {
        ...
        var base = {
            'source': 'widget',
            'version': self.maxuiactivity.version,
            'user': {
                'username': self.maxuiactivity.settings.username,
                'displayname': self.maxuiactivity.settings.displayName
            },
            'published': self.maxuiactivity.utils.rfc3339(self.maxuiactivity.utils.now()),
            'uuid': uuid.v1()
        };
        if (self.domain) {
    ...
    MaxMessaging.prototype.send = function(message, routing_key) {
        ...
        self.stomp.send('/exchange/{0}.publish/{1}'.format(self.maxuiactivity.settings.username, routing_key), {}, JSON.stringify(self.pack(message_unpacked)));
...
(function(jq) {
    ...
    jq.fn.maxUIActivity = function(options) {
        ...
        var maxuiactivity = this;
        maxuiactivity.version = '4.1.21';
        maxuiactivity.templates = max.templates();
        maxuiactivity.utils = max.utils();
        var defaults = {
        ...
        maxuiactivity.settings = jq.extend(defaults, options);
        maxuiactivity.logger = new max.MaxLogging(maxuiactivity);
        maxuiactivity.logger.setLevel(maxuiactivity.settings.loglevel);
        // Configure maxui without CORS if CORS not available
        if (!maxuiactivity.utils.isCORSCapable()) {
            // IF it has been defined an alias, set as max server url
            if (maxuiactivity.settings.maxServerURLAlias) {
                maxuiactivity.settings.maxServerURL = maxuiactivity.settings.maxServerURLAlias;
            }
        }
        ...
        maxuiactivity.settings.maxTalkURL = maxuiactivity.utils.normalizeWhiteSpace(maxuiactivity.settings.maxTalkURL);
        if (_.isUndefined(maxuiactivity.settings.maxTalkURL) || maxuiactivity.settings.maxTalkURL === "") {
            maxuiactivity.settings.maxTalkURL = maxuiactivity.settings.maxServerURL + '/stomp';
        }
        // Normalize domain if present, to avoid errors with unvalid values and whitespaces
        maxuiactivity.settings.domain = maxuiactivity.utils.normalizeWhiteSpace(maxuiactivity.settings.domain);
        if (maxuiactivity.settings.domain.toLowerCase() === 'none') {
            maxuiactivity.settings.domain = "";
        }
        // Check timeline/activities consistency
        if (maxuiactivity.settings.UISection === 'timeline' && maxuiactivity.settings.activitySource === 'timeline' && maxuiactivity.settings.readContext) {
            maxuiactivity.settings.readContext = undefined;
            maxuiactivity.settings.writeContexts = [];
        }
        // Check showSubscriptionList consistency
        if (maxuiactivity.settings.showSubscriptionList && maxuiactivity.settings.activitySource === 'activities') {
            maxuiactivity.settings.showSubscriptionList = false;
        }
        // Get language from options or set default.
        // Set literals in the choosen language and extend from user options
        maxuiactivity.language = options.language || 'en';
        var user_literals = options.literals || {};
        maxuiactivity.settings.literals = jq.extend(max.literals(maxuiactivity.language), user_literals);
        if (maxuiactivity.settings.readContext) {
            // Calculate readContextHash
            maxuiactivity.settings.readContextHash = maxuiactivity.utils.sha1(maxuiactivity.settings.readContext);
            // Add read context to write contexts
            maxuiactivity.settings.writeContexts.push(maxuiactivity.settings.readContext);
            // Store the hashes of the write contexts
            maxuiactivity.settings.writeContextsHashes = [];
            for (var wc = 0; wc < maxuiactivity.settings.writeContexts.length; wc++) {
                maxuiactivity.settings.writeContextsHashes.push(maxuiactivity.utils.sha1(maxuiactivity.settings.writeContexts[wc]));
            }
        }
        //set default avatar and profile url pattern if user didn't provide it
        if (!maxuiactivity.settings.avatarURLpattern) {
            maxuiactivity.settings.avatarURLpattern = maxuiactivity.settings.maxServerURL + '/people/{0}/avatar';
        }
        if (!maxuiactivity.settings.contextAvatarURLpattern) {
            maxuiactivity.settings.contextAvatarURLpattern = maxuiactivity.settings.maxServerURL + '/contexts/{0}/avatar';
        }
        if (!maxuiactivity.settings.conversationAvatarURLpattern) {
            maxuiactivity.settings.conversationAvatarURLpattern = maxuiactivity.settings.maxServerURL + '/conversations/{0}/avatar';
        }
        // Disable profileURL by now
        // if (!maxuiactivity.settings.profileURLpattern)
        //        maxuiactivity.settings['profileURLpattern'] = maxuiactivity.settings.maxServerURL+'/profiles/{0}'
        // Catch errors triggered by failed max api calls
        if (maxuiactivity.settings.enableAlerts) {
        ...
        // Init MAX Client
        maxuiactivity.maxClient = new MaxClient();
        var maxclient_config = {
            server: maxuiactivity.settings.maxServerURL,
            mode: maxuiactivity.settings.maxRequestsAPI,
            username: maxuiactivity.settings.username,
            token: maxuiactivity.settings.oAuthToken
        };
        maxuiactivity.maxClient.configure(maxclient_config);
        // Create a instance of a max messaging client
        // This needs to be before MaxConversations instance creation
        maxuiactivity.messaging = new max.MaxMessaging(maxuiactivity);
        // View representing the conversations section
        // Make settings available to utils package
        maxuiactivity.utils.setSettings(maxuiactivity.settings);
        /**
         *
         *
         **/
        jq.fn.hidePostbox = function() {
            var maxuiactivity = this;
            if (maxuiactivity.settings.activitySource === 'timeline') {
                if (maxuiactivity.settings.subscriptionsWrite.length > 0) {
                    return maxuiactivity.settings.hidePostboxOnTimeline;
                }
            }
            for (var i in maxuiactivity.settings.subscriptionsWrite) {
                var hash = maxuiactivity.settings.subscriptionsWrite[i].hash;
                if (hash === maxuiactivity.settings.readContext) {
                    return maxuiactivity.settings.hidePostboxOnTimeline;
                }
            }
            return true;
        ...
        this.maxClient.getUserData(maxuiactivity.settings.username, function(data) {
            //Determine if user can write in writeContexts
            maxuiactivity.settings.displayName = data.displayName || maxuiactivity.settings.username;
            ...
            maxuiactivity.settings.subscriptionsWrite = subscriptionsWrite;
            maxuiactivity.settings.subscriptions = userSubscriptions;
            if (maxuiactivity.settings.activitySource === 'timeline' && subscriptionsWrite.length > 0) {
                maxuiactivity.settings.writeContexts.push(subscriptionsWrite[0].hash);
            }
            // Start messaging only if conversations enabled
            if (!maxuiactivity.settings.disableConversations) {
                maxuiactivity.messaging.start();
                jq(window).on('beforeunload', function(event) {
                    var x = maxuiactivity.messaging.disconnect();
                    return x;
                });
            }
            // render main interface
            var showCT = maxuiactivity.settings.UISection === 'conversations';
            var showTL = maxuiactivity.settings.UISection === 'timeline';
            var toggleTL = maxuiactivity.settings.disableTimeline === false && !showTL;
            var toggleCT = maxuiactivity.settings.disableConversations === false && !showCT;
            var containerWidth = maxuiactivity.width() - maxuiactivity.settings.scrollbarWidth;
            var showRecentOrder = maxuiactivity.settings.activitySortView === 'recent';
            var showLikesOrder = maxuiactivity.settings.activitySortView === 'likes';
            var showFlaggedOrder = maxuiactivity.settings.activitySortView === 'flagged';
            var params_activity = {
                username: maxuiactivity.settings.username,
                literals: maxuiactivity.settings.literals,
                ...
                hidePostbox: maxuiactivity.hidePostbox()
            };
            var mainuiactivity = maxuiactivity.templates.mainUIActivity.render(params_activity);
            maxuiactivity.html(mainuiactivity);
            maxuiactivity.overlay = new max.views.MaxOverlay(maxuiactivity);
            // Define widths
            // XXX TODO :Read from renderer styles, not hardcoded values
            maxuiactivity.settings.widgetWidth = maxuiactivity.width();
            maxuiactivity.settings.sectionsWidth = maxuiactivity.settings.widgetWidth - maxuiactivity.settings.scrollbarWidth - maxuiactivity.settings.widgetBorder;
            // First-rendering of conversations list, even if it's not displayed on start
            if (!maxuiactivity.settings.disableConversations) {
                maxuiactivity.conversations.render();
                maxuiactivity.conversations.listview.load(data.talkingIn);
            }
            if (maxuiactivity.settings.UISection === 'conversations') {
                maxuiactivity.bindEvents();
                maxuiactivity.toggleSectionActivity('conversations');
            } else if (maxuiactivity.settings.UISection === 'timeline') {
                var sort_orders_by_view = {
                    recent: maxuiactivity.settings.activitySortOrder,
                    likes: 'likes',
                    flagged: 'flagged'
                };
                maxuiactivity.printActivities({
                    sortBy: sort_orders_by_view[maxuiactivity.settings.activitySortView]
                }, function(event) {
                    maxuiactivity.bindEvents();
                });
            }
    ...
    jq.fn.bindEvents = function() {
        ...
        jq('#maxuiactivity-widget-container #maxui-more-activities .maxui-button').click(function(event) {
            ...
            if (jq('#maxuiactivity-widget-container #maxui-search').hasClass('folded')) {
                maxuiactivity.loadMoreActivities();
            } else {
                var last_result_id = jq('.maxui-activity:last').attr('id');
                maxuiactivity.reloadFilters(last_result_id);
            }
        });
        ...
        jq('#maxuiactivity-widget-container #maxui-news-activities .maxui-button').click(function(event) {
            maxuiactivity.loadNewsActivities();
        });
        ...
        jq('#maxuiactivity-widget-container #maxui-search').on('click', '#maxui-favorites-filter', function(event) {
            ...
            if (filterFavorites) {
                ...
                valued_literal = maxuiactivity.settings.literals.valued_favorited_activity;
                recent_literal = maxuiactivity.settings.literals.recent_favorited_activity;
            } else {
                maxuiactivity.delFilter({
                    type: 'favorites',
                    value: true
                });
                valued_literal = maxuiactivity.settings.literals.valued_activity;
                recent_literal = maxuiactivity.settings.literals.recent_activity;
            }
        ...
        jq('#maxuiactivity-widget-container #maxui-subscriptions').on('change', function() {
            ...
            if ($urlContext !== 'timeline') {
                ...
                maxuiactivity.settings.writeContextsHashes = [];
                // Add read context to write contexts
                maxuiactivity.settings.writeContexts.push($urlContext);
                // Store the hashes of the write contexts
                for (var wc = 0; wc < maxuiactivity.settings.writeContexts.length; wc++) {
                    maxuiactivity.settings.writeContextsHashes.push(maxuiactivity.utils.sha1(maxuiactivity.settings.writeContexts[wc]));
                }
                ...
                maxuiactivity.settings.writeContexts = [];
                maxuiactivity.settings.writeContextsHashes = undefined;
            }
        });
        ...
        jq('#maxuiactivity-widget-container #maxui-show-timeline').on('click', function(event) {
            event.preventDefault();
            window.status = '';
            maxuiactivity.printActivities({}, function(event) {
                maxuiactivity.toggleSectionActivity('timeline');
            });
        });
        ...
        jq('#maxuiactivity-widget-container #maxui-activities').on('click', '.maxui-action.maxui-favorites', function(event) {
            ...
            if (favorited) {
                maxuiactivity.maxClient.unfavoriteActivity(activityid, function(event) {
                    $favorites.toggleClass('maxui-favorited', false);
                });
            } else {
                maxuiactivity.maxClient.favoriteActivity(activityid, function(event) {
        ...
        jq('#maxuiactivity-widget-container #maxui-activities').on('click', '.maxui-action.maxui-likes', function(event) {
            ...
            if (liked) {
                maxuiactivity.maxClient.unlikeActivity(activityid, function(event) {
                    $likes.toggleClass('maxui-liked', false);
                });
                $likes_count.text(parseInt($likes_count.text(), 10) - 1);
            } else {
                maxuiactivity.maxClient.likeActivity(activityid, function(event) {
        ...
        jq('#maxuiactivity-widget-container #maxui-activities').on('click', '.maxui-action.maxui-flag', function(event) {
            ...
            if (flagged) {
                maxuiactivity.maxClient.unflagActivity(activityid, function(event) {
                    $flag.toggleClass('maxui-flagged', false);
                    $activity.toggleClass('maxui-flagged', false);
                    maxuiactivity.printActivities({});
                });
            } else {
                maxuiactivity.maxClient.flagActivity(activityid, function(event) {
        ...
        jq('#maxuiactivity-widget-container #maxui-activities').on('click', '.maxui-actions .maxui-button-delete', function(event) {
            ...
            var $activity = jq(this).closest('.maxui-activity');
            var activityid = $activity.attr('id');
            maxuiactivity.maxClient.removeActivity(activityid, function(event) {
        ...
        jq('#maxuiactivity-widget-container #maxui-activities').on('click', '.maxui-comment .maxui-button-delete', function(event) {
            ...
            maxuiactivity.maxClient.removeActivityComment(activityid, commentid, function() {
        ...
        jq('#maxuiactivity-widget-container #maxui-add-people-box').on('focusin', selector, function(event) {
            ...
            var normalized = maxuiactivity.utils.normalizeWhiteSpace(text, false);
            ...
            if (jq('#maxuiactivity-widget-container #maxui-conversation-predictive:visible').length > 0 && (event.which === 40 || event.which === 38 || event.which === 13 || event.which === 9)) {
                maxuiactivity.utils.freezeEvent(event);
            }
            ...
            if (normalized === '') {
                ...
                if (maxuiactivity.settings.canwrite) {
            ...
            {
                if (maxuiactivity.settings.conversationsSection === 'conversations') {
                    if (match) {
                        $area.attr('class', 'maxui-text-input');
                        if (matchEOL) {
                            $predictive.show();
                            $predictive.html('<ul></ul>');
                            maxuiactivity.printPredictions(match[1], '#maxui-conversation-predictive');
                        }
                    } else {
                        $predictive.hide();
                        ...
                        if (!text.match(new RegExp('^\\s*@'))) {
                            ...
                            $area.attr('title', maxuiactivity.settings.literals.post_permission_not_here);
                        }
                    }
                }
            } //1
        ...
        maxuiactivity.bindActionBehaviour('#maxui-newactivity', '#maxui-newactivity-box', {}, function(text) {
            if (maxuiactivity.settings.UISection === 'timeline') {
                maxuiactivity.sendActivity(text);
                jq('#maxuiactivity-widget-container #maxui-search').toggleClass('folded', true);
            } else if (maxuiactivity.settings.UISection === 'conversations') {
                if (maxuiactivity.settings.conversationsSection === 'conversations') {
                    ...
                    maxuiactivity.conversations.create(options);
                } else {
                    maxuiactivity.conversations.send(text);
                }
            }
            ...
            if (key === 40 && is_predicting && num_predictions > 1) {
                ...
                if (maxuiactivity.settings.UISection === 'timeline') {
                    jq(button).val(maxuiactivity.settings.literals.new_activity_post);
                }
            } else //1
            {
                if (maxuiactivity.settings.UISection === 'timeline') {
                    if (match) {
                        jq(button).val(maxuiactivity.settings.literals.new_message_post);
                        if (matchEOL) {
                            $predictive.show();
                            $predictive.html('<ul></ul>');
                            maxuiactivity.printPredictions(match[1], '#maxui-newactivity #maxui-predictive');
                        }
                        ...
                        jq(button).val(maxuiactivity.settings.literals.new_activity_post);
                        $predictive.hide();
                        if (!text.match(new RegExp('^\\s*@')) && !maxuiactivity.settings.canwrite) {
                            $area.attr('class', 'maxui-text-input error');
                            $area.attr('title', maxuiactivity.settings.literals.post_permission_unauthorized);
                        }
                    }
                } else if (maxuiactivity.settings.UISection === 'conversations') {
                    if (maxuiactivity.settings.conversationsSection === 'conversations') {
                        maxuiactivity.reloadPersons();
                    } else if (maxuiactivity.settings.conversationsSection === 'messages') {
                        $predictive.hide();
                        jq(button).removeAttr('disabled');
                        jq(button).attr('class', 'maxui-button');
                        $area.attr('class', 'maxui-text-input');
        ...
        maxuiactivity.bindActionBehaviour('#maxui-activities', '.maxui-newcommentbox', {}, function(text) {
            var activityid = jq(this).closest('.maxui-activity').attr('id');
            maxuiactivity.maxClient.addComment(text, activityid, function() {
                jq('#maxuiactivity-widget-container #activityContainer textarea').val('');
                var activity_id = this.object.inReplyTo[0].id;
                maxuiactivity.printCommentsForActivity(activity_id);
        ...
        maxuiactivity.bindActionBehaviour('#maxui-search', '#maxui-search-box', {}, function(text) {
            maxuiactivity.textSearch(text);
            jq('#maxuiactivity-widget-container #maxui-search').toggleClass('folded', false);
        ...
        //           if (e.keyCode === 13) {
        //              maxuiactivity.textSearch(jq(this).attr('value'))
        //              jq('#maxuiactivity-widget-container #maxui-search').toggleClass('folded',false)
        //           }
        // });
    };
    ...
    jq.fn.bindActionBehaviour = function(delegate, target, options, clickFunction) {
        ...
        jq(delegate).on('focusin', selector, function(event) {
            ...
            if (normalized === '' && !options.ignore_button) {
                ...
                if (maxuiactivity.settings.canwrite && !options.ignore_button) {
                    jq(button).removeAttr('disabled');
                    jq(button).attr('class', 'maxui-button');
                    jq(this).attr('class', 'maxui-text-input');
                }
    ...
    jq.fn.textSearch = function(text) {
        var maxuiactivity = this;
        //Normalize spaces
        var normalized = maxuiactivity.utils.normalizeWhiteSpace(text);
    ...
    jq.fn.reloadFilters = function() {
        ...
        var activity_items = maxuiactivity.templates.filters.render(params);
        jq('#maxuiactivity-widget-container #maxui-search-filters').html(activity_items);
        // Accept a optional parameter indicating search start point
        if (arguments.length > 0) {
            filters = maxuiactivity.getFilters(arguments[0]);
        } else {
            filters = maxuiactivity.getFilters();
        }
        maxuiactivity.printActivities({});
    ...
    jq.fn.delFilter = function(filter) {
        var maxuiactivity = this;
        var deleted = false;
        for (var i = 0; i < maxuiactivity.filters.length; i++) {
            if (maxuiactivity.filters[i].value === filter.value & maxuiactivity.filters[i].type === filter.type) {
                deleted = true;
                maxuiactivity.filters.splice(i, 1);
            }
        }
        if (deleted) {
            this.reloadFilters();
    ...
    jq.fn.addFilter = function(filter) {
        ...
        if (!maxuiactivity.filters) {
            maxuiactivity.filters = [];
        }
        ...
        if (!already_filtered) {
            maxuiactivity.filters.push(filter);
            if (reload === true) {
                this.reloadFilters();
            }
        }
    ...
    jq.fn.reloadPersons = function() {
        ...
        message = maxuiactivity.utils.normalizeWhiteSpace(message);
        ...
        displayName = maxuiactivity.utils.normalizeWhiteSpace(displayName);
        var params = {
            persons: participants_box.people
        };
        var participants_items = maxuiactivity.templates.participants.render(params);
        jq('#maxuiactivity-widget-container #maxui-new-participants').html(participants_items);
        jq('#maxuiactivity-widget-container #maxui-add-people-box .maxui-label .maxui-count').text('({0}/{1})'.format(participants_box.people.length + 1, maxuiactivity.settings.maximumConversations));
        if (participants_box.people.length > 0) {
            ...
            if (has_more_than_one_participant && !has_a_displayname) {
                ...
                if (displayName === '') {
                    ...
                    $newmessagebox.find('.maxui-error-box').text(maxuiactivity.settings.literals.post_permission_missing_displayName);
            ...
            $newmessagebox.find('textarea').attr('class', 'maxui-text-input error');
            $newmessagebox.find('.maxui-error-box').text(maxuiactivity.settings.literals.post_permission_not_enough_participants);
            $newmessagebox.find('.maxui-error-box').width($newmessagebox.find('textarea').width() - 4);
            $newmessagebox.find('.maxui-error-box').animate({
                'bottom': -25
            }, 200);
    ...
    jq.fn.addPerson = function(person) {
        ...
        if (person.username !== maxuiactivity.settings.username && participants_box.people.length < (maxuiactivity.settings.maximumConversations - 1)) {
            for (var i = 0; i < participants_box.people.length; i++) {
                if (participants_box.people[i].username === person.username) {
                    already_filtered = true;
                }
    ...
    jq.fn.toggleSectionActivity = function(sectionToEnable) {
        ...
        var widgetWidth = maxuiactivity.width();
        var sectionPadding = 10;
        var widgetBorder = 1;
        var sectionsWidth = widgetWidth - maxuiactivity.conversations.scrollbar.width - (sectionPadding * 2) - (widgetBorder * 2);
        var height = 320;
        if (sectionToEnable === 'conversations' && maxuiactivity.settings.currentConversationSection === 'conversations') {
            ...
            maxuiactivity.settings.UISection = 'conversations';
            $postbutton.val(maxuiactivity.settings.literals.new_message_post);
            textarea_literal = maxuiactivity.settings.literals.new_conversation_text;
            $postbox_text.val(textarea_literal).attr('data-literal', textarea_literal);
            $conversationsbutton.hide();
            if (!maxuiactivity.settings.disableTimeline) {
                $timelinebutton.show();
            }
            maxuiactivity.conversations.scrollbar.setHeightActivity(height - 45);
            maxuiactivity.conversations.scrollbar.setTarget('#maxui-conversations #maxui-conversations-list');
            $postbox.show();
        }
        if (sectionToEnable === 'timeline') {
            if (maxuiactivity.settings.showSubscriptionList === true) {
                $subscriptionsSelect.attr('style', 'display:inline;');
            }
            maxuiactivity.conversations.listview.toggle();
            $timeline.show();
            ...
            maxuiactivity.settings.UISection = 'timeline';
            $postbutton.val(maxuiactivity.settings.literals.new_activity_post);
            textarea_literal = maxuiactivity.settings.literals.new_activity_text;
            $postbox_text.val(textarea_literal).attr('data-literal', textarea_literal);
            if (!maxuiactivity.settings.disableConversations) {
                $conversationsbutton.show();
            }
            $timelinebutton.hide();
            if (maxuiactivity.settings.hidePostboxOnTimeline) {
                $postbox.hide();
            }
            if (maxuiactivity.hidePostbox()) {
                $postbox.hide();
            }
        }
    };
    ...
    jq.fn.sendActivity = function() {
        ...
        func_params.push(maxuiactivity.settings.writeContexts);
        func_params.push(function() {
            ...
            if (first.length > 0) {
                ...
                maxuiactivity.printActivities(filter);
            } else {
                maxuiactivity.printActivities({});
            }
        });
        //Pass generator to activity post if defined
        if (maxuiactivity.settings.generatorName) {
            func_params.push(maxuiactivity.settings.generatorName);
        }
        var activityAdder = maxuiactivity.maxClient.addActivity;
        activityAdder.apply(maxuiactivity.maxClient, func_params);
    };
    /**
    ...
    jq.fn.printPredictions = function(query, predictive_selector) {
        ...
        func_params.push(function(items) {
            maxuiactivity.formatPredictions(items, predictive_selector);
        });
    ...
    jq.fn.formatPredictions = function(items, predictive_selector) {
        ...
        for (var i = 0; i < items.length; i++) {
            var prediction = items[i];
            if (prediction.username !== maxuiactivity.username) {
                var avatar_url = maxuiactivity.settings.avatarURLpattern.format(prediction.username);
                var params = {
                    username: prediction.username,
                    displayName: prediction.displayName,
                    avatarURL: avatar_url,
                ...
                predictions = predictions + maxuiactivity.templates.predictive.render(params);
            }
        }
        if (predictions === '') {
            predictions = '<li>' + maxuiactivity.settings.literals.no_match_found + '</li>';
        }
    ...
    jq.fn.formatActivities = function(items, insertAt) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (activity.hasOwnProperty('generator')) {
                if (activity.generator !== maxuiactivity.settings.generatorName) {
            ...
            if (activity.actor.objectType === 'person') {
                avatar_url = maxuiactivity.settings.avatarURLpattern.format(activity.actor.username);
            } else if (activity.actor.objectType === 'context') {
                avatar_url = maxuiactivity.settings.contextAvatarURLpattern.format(activity.actor.hash);
            }
                ...
                if (activity.replies.length > 0) {
                    for (var r = 0; r < activity.replies.length; r++) {
                        ...
                        var reply = {
                            id: comment.id,
                            actor: comment.actor,
                            date: maxuiactivity.utils.formatDate(comment.published, maxuiactivity.language),
                            text: maxuiactivity.utils.formatText(comment.content),
                            avatarURL: maxuiactivity.settings.avatarURLpattern.format(comment.actor.username),
            ...
            var canCommentActivity = maxuiactivity.settings.canwrite;
            if (activity.contexts) {
                canCommentActivity = maxuiactivity.canCommentActivity(activity.contexts[0].url);
            }
        }
        ...
        _.each(images_to_render, function(activity, index, list) {
            maxuiactivity.maxClient.getMessageImage('/activities/{0}/image/thumb'.format(activity.id), function(encoded_image_data) {
    ...
    jq.fn.formatComment = function(items, activity_id) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            comments = comments + maxuiactivity.templates.comment.render(params);
        }
    ...
    jq.fn.renderPostbox = function() {
        ...
        var toggleCT = maxuiactivity.settings.disableConversations === false && !showCT;
        var params = {
            avatar: maxuiactivity.settings.avatarURLpattern.format(maxuiactivity.settings.username),
            allowPosting: maxuiactivity.settings.canwrite,
            buttonLiteral: maxuiactivity.settings.literals.new_activity_post,
            textLiteral: maxuiactivity.settings.literals.new_activity_text,
            literals: maxuiactivity.settings.literals,
            showConversationsToggle: toggleCT ? 'display:block;' : 'display:none;',
            showSubscriptionList: maxuiactivity.settings.showSubscriptionList && maxuiactivity.settings.subscriptionsWrite.length > 0,
            subscriptionList: maxuiactivity.settings.subscriptionsWrite
        };
        var postbox = maxuiactivity.templates.postBox.render(params);
        var $postbox = jq('#maxuiactivity-widget-container #maxui-newactivity');
    ...
    jq.fn.printActivities = function(ufilters) {
        ...
        var filters = maxuiactivity.getFilters().filters;
        ...
        if (!filters.sortBy) {
            if (jq('#maxuiactivity-widget-container #maxui-activity-sort .maxui-sort-action.maxui-most-valued').hasClass('active')) {
                ...
                filters.sortBy = maxuiactivity.settings.activitySortOrder;
            }
        }
        maxuiactivity.currentSortOrder = filters.sortBy;
        var activityRetriever = null;
        if (maxuiactivity.settings.activitySource === 'timeline') {
            activityRetriever = this.maxClient.getUserTimeline;
            func_params.push(maxuiactivity.settings.username);
        } else if (maxuiactivity.settings.activitySource === 'activities') {
            activityRetriever = this.maxClient.getActivities;
            var options = {
                context: maxuiactivity.settings.readContextHash,
                tags: maxuiactivity.settings.contextTagsFilter
            };
        ...
        if (arguments.length > 1) {
            ...
            func_params.push(function(items) {
                ...
                maxuiactivity.settings.canwrite = true;
                // If we don't have a context, we're in timeline, so we can write
                if (maxuiactivity.settings.activitySource === 'activities') {
                    maxuiactivity.maxClient.getContext(maxuiactivity.settings.readContextHash, function(context) {
                        // Add read context if user is not subscribed to it
                        var subscriptions = maxuiactivity.settings.subscriptions;
                        ...
                        // the current user
                        for (var wc = 0; wc < maxuiactivity.settings.writeContexts.length; wc++) {
                            var write_context = maxuiactivity.settings.writeContextsHashes[wc];
                            if (subscriptions[write_context].permissions) {
                                if (subscriptions[write_context].permissions.write !== true) {
                                    maxuiactivity.settings.canwrite = false;
                                }
                                if (subscriptions[write_context].permissions.flag === true) {
                                    maxuiactivity.settings.canflag = true;
                                } else {
                                    maxuiactivity.settings.canflag = false;
                                }
                            } else {
                                maxuiactivity.settings.canwrite = false;
                                maxuiactivity.settings.canflag = false;
                        ...
                        maxuiactivity.renderPostbox();
                        // format the result items as activities
                        maxuiactivity.formatActivities(items, insert_at, callback);
                    });
                } else {
                    maxuiactivity.renderPostbox(items, insert_at, callback);
                    // format the result items as activities
                    maxuiactivity.formatActivities(items, insert_at, callback);
                }
            });
            ...
            func_params.push(function(items) {
                maxuiactivity.formatActivities(items, insert_at);
            });
        }
    ...
    jq.fn.printCommentsForActivity = function(activity_id) {
        ...
        func_params.push(function(data) {
            maxuiactivity.formatComment(data, activity_id);
        });
        this.maxClient.getCommentsForActivity.apply(this.maxClient, func_params);
    };
    jq.maxuiactivity = function() {};
    jq.maxuiactivity.settings = function() {
...
// This snippet assumes that a _maxuiactivity.onReady function is defined by the api consumer and calls it as a final step
